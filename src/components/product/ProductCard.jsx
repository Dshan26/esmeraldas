...
/**
 * Product card component for the emerald catalog.
 * Displays product image (lazy loaded), name, short description,
 * carats, formatted COP price, and a WhatsApp buy button.
 *
...
export default function ProductCard({ product }) {
  ...
  return (
    <article className={styles.card} onClick={handleCardClick} data-testid="product-card">
      <div className={styles.imageWrapper}>
        <LazyImage src={mainImage} alt={productName} />
      </div>
      <div className={styles.body}>
        <h3 className={styles.name}>{productName}</h3>
        <p className={styles.description}>{shortDescription}</p>
        <p className={styles.carats}>
          {product.carats} {t('product.carats')}
        </p>
